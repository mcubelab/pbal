...
xg = [x_c; y_c; x; 0; 0; 0]; % goal state
ug = [0; p_guess.m * p_guess.g;
    0.5 * p_guess.m * p_guess.g * p_guess.l*sin(x)];
...
for k = 1:mpc_tv.Ntraj
    ...
    [dXdt_guess,dPdt]= p_guess.extended_kalmann_update(Z,X_guess,uk,P,Q,R);
...
figure(1); clf;
hold on; axis equal;
xlim(x_c + [-2*p.l, 2*p.l])
ylim(y_c + [-2*p.l, 2*p.l])
...
for k = 1:(p.nq + p.nv)
    ...
    if k == 3
        plot(t, multiplier(k) * X_guess(1, :))
    end
    
    if k == 6
        plot(t, multiplier(k) * X_guess(2, :))
    end        
