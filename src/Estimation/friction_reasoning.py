...
def compute_sliding_state_world_manipulation(sliding_state_dict,friction_parameter_dict,last_slide_time_dict,t0,measured_world_manipulation_wrench,external_friction_cone_boundary_margin,reset_time_length):
    ...
    if len(friction_parameter_dict['aer'])>0:
        slide_right_bool = any(np.dot(friction_parameter_dict['aer'],measured_world_manipulation_wrench) > friction_parameter_dict['ber'] - external_friction_cone_boundary_margin)
    else:
        slide_right_bool = True

    if len(friction_parameter_dict['ael'])>0:
        slide_left_bool = any(np.dot(friction_parameter_dict['ael'],measured_world_manipulation_wrench) > friction_parameter_dict['bel'] - external_friction_cone_boundary_margin)
    else:
