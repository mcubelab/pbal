...
from Estimation.test.gtsam_shape_contact_estimator import gtsam_shape_contact_estimator
...
if __name__ == '__main__':
    ...
    if shape_name == 'big_rectangle':
        temp0 = np.array(object_vertex_array[0])
        temp1 = np.array(object_vertex_array[1])

        object_vertex_array[0]*=0
        object_vertex_array[1]*=0

        object_vertex_array[0]+=temp1-.065
        object_vertex_array[1]+=temp0-.1

    dummy, apriltag_id, apriltag_pos = ioh.load_shape_data(shape_name)
    ...
    for count in range(len(data_dict['far_cam/color/image_raw'])):
        ...
        if 'friction_parameters' in synched_data_dict.keys():
            ...
            if count%1==0:
                ...
                my_shape_contact_estimator.add_data_point(hand_pose_pivot_estimator,measured_wrench_pivot_estimator,sliding_state_dict)
            if count%1==0 and my_pivot_estimator.num_data_points>20:
                ...
                # pivot_estimate_new = my_shape_contact_estimator.compute_estimate()
                pivot_estimate_vector = np.array([[-pivot_estimate_new[0],hand_front_center_world[1],pivot_estimate_new[1],1]])
            ...
            if pivot_estimate_vector is not None:
                # temp_vertex_array = np.zeros([len(object_vertex_array),len(object_vertex_array[0])])
                # temp_vertex_array[3][:]= object_vertex_array[3][:]
                # temp_vertex_array[2][:]+= .041
                # temp_vertex_array[0][:]=-object_vertex_array[1][:]
                # temp_vertex_array[1][:]= object_vertex_array[0][:]-pivot_estimate_new[2]
                ...
                # temp_vertex_array = np.zeros([len(object_vertex_array),1])
                # temp_vertex_array[3][:]= 1.0
                # temp_vertex_array[2][:]+= .041
                # temp_vertex_array[0][:]= 0.0
                # temp_vertex_array[1][:]= -pivot_estimate_new[2] 
                # contact_estimate_vector = np.dot(contact_pose_homog,temp_vertex_array)
                # contact_estimate_vector = contact_estimate_vector.transpose()
                # ioh.plot_pivot_dot(cv_image,contact_estimate_vector,camera_transformation_matrix)

                # temp_vertex_array = np.zeros([len(object_vertex_array),1])
                # temp_vertex_array[3][:]= 1.0
                # temp_vertex_array[2][:]+= .041
                # temp_vertex_array[0][:]= pivot_estimate_new[4]
                # temp_vertex_array[1][:]= -pivot_estimate_new[2] + pivot_estimate_new[3]
                # com_estimate_vector = np.dot(contact_pose_homog,temp_vertex_array)

                # com_estimate_vector = com_estimate_vector.transpose()
                # ioh.plot_pivot_dot(cv_image,com_estimate_vector,camera_transformation_matrix)
        ...
        if measured_contact_wrench_6D is not None and measured_base_wrench_6D is not None:
            if np.abs(measured_contact_wrench_6D[0]) > .1:
                ...
                # vertex_array_out = np.dot(contact_pose_homog,object_vertex_array)
