...
if __name__ == '__main__':
    ...
    with open('RobotDelayData13.txt', 'r') as f:
    ...
    impedance_target_horizontal_pose_list = np.array(impedance_target_horizontal_pose_list[min_index:max_index])
    measured_horizontal_pose_list = np.array(measured_horizontal_pose_list[min_index:max_index])
    impedance_target_vertical_pose_list = np.array(impedance_target_vertical_pose_list[min_index:max_index])
    measured_vertical_pose_list = np.array(measured_vertical_pose_list[min_index:max_index])
    measured_horiztonal_force_list = np.array(measured_horiztonal_force_list[min_index:max_index])
    measured_vertical_force_list = np.array(measured_vertical_force_list[min_index:max_index])
    ...
    Coeff_Vec_horz = np.linalg.lstsq(np.transpose(regression_matrix),measured_horiztonal_force_list)[0]
    Coeff_Vec_vert = np.linalg.lstsq(np.transpose(regression_matrix),measured_vertical_force_list)[0]

    delay_horz =  np.arctan2(-Coeff_Vec_horz[0],Coeff_Vec_horz[1])/(2*np.pi/period)
    ...
    delay_vert =  np.arctan2(Coeff_Vec_vert[1],Coeff_Vec_vert[0])/(2*np.pi/period)
    ...
    axs[0].plot(tlist,impedance_target_horizontal_pose_list,marker='o',color='b', label='target')
    axs[0].plot(tlist,measured_horizontal_pose_list,marker='o',color='r', label='measured')

    axs[1].plot(tlist,impedance_target_vertical_pose_list,marker='o',color='b', label='target')
    axs[1].plot(tlist,measured_vertical_pose_list,marker='o',color='r', label='measured')
    ...
    axs[2].plot(tlist,Coeff_Vec_horz[2] + sine_wave(tlist-delay_horz,amplitude_horz,period),color='r')

    axs[3].plot(tlist,measured_vertical_force_list,color='b')
    axs[3].plot(tlist,Coeff_Vec_vert[2] + cos_wave(tlist-delay_vert,amplitude_vert,period),color='r')
