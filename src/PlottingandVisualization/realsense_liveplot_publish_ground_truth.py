    ...
    while not rospy.is_shutdown():
        ...
        if friction_parameter_list:
            while friction_parameter_list:
                ...
                P0_L = [
                    A_contact_left[0] * B_contact_left,
                    A_contact_left[1] * B_contact_left
                ]
                P0_R = [
                    A_contact_right[0] * B_contact_right,
                    A_contact_right[1] * B_contact_right
                ]
        ...
        if current_image is not None:
            ...
            # ros_helper.list2pose_stamped(endpoint_pose_list)
            ...
            if (measured_contact_wrench_6D is not None
                ...
                if friction_parameter_dict is not None:
                    ...
                    B_list = np.hstack(
                        [B_external_right, B_external_left, 40.0])
                ...
                if measured_contact_wrench_6D is not None and measured_base_wrench_6D is not None:
                    ...
                    if P0 is not None:
                        ...
                        if friction_parameter_dict is not None:
                            ...
                            myHullVertexListX = -force_scale * np.array(
                                myHullVertexListX) + P0[0]
                            myHullVertexListY = -force_scale * np.array(
                                myHullVertexListY) + P0[2]

                            numPts = len(myHullVertexListX)
                            BoundaryPts = np.vstack([
                                myHullVertexListX,
                                P0[1] * np.ones([1, numPts]),
                                myHullVertexListY,
                                np.ones([1, numPts])
                            ])
        ...
        if measured_contact_wrench_6D is not None and measured_base_wrench_6D is not None:
            if np.abs(measured_contact_wrench_6D[0]) > .1:
                ...
                if friction_parameter_dict is not None:
                    ...
                    left_bound0 = mid_point_hand_frame + np.array(
                        [x_left0, y_left0, 0.0, 0.0])
                    left_bound1 = mid_point_hand_frame + np.array(
                    ...
                    right_bound0 = mid_point_hand_frame + np.array(
                        [x_right0, y_right0, 0.0, 0.0])
                    right_bound1 = mid_point_hand_frame + np.array(
