...
if __name__ == '__main__':
	...
	hand_2D_normal_world_prev = None
	hand_2D_tangent_world_prev = None
	hand_2D_midpoint_world_prev = None
	...
	while  count <len(data_dict['tag_detections']):
		...
		if count>0:
			hand_2D_normal_world_prev = hand_2D_normal_world
			hand_2D_tangent_world_prev = hand_2D_tangent_world
			hand_2D_midpoint_world_prev = hand_2D_midpoint_world
		...
		hand_points_world = np.dot(contact_pose_homog,hand_points)
		hand_midpoint_world = np.dot(contact_pose_homog,hand_midpoint)
		hand_2D_midpoint_world = np.array([hand_midpoint_world[0],hand_midpoint_world[2]])
		...
		if count>0:
			ICR_M1 = np.transpose(np.vstack([hand_2D_tangent_world-hand_2D_tangent_world_prev,hand_2D_normal_world-hand_2D_normal_world_prev]))
			ICR_M2 = np.transpose(np.vstack([hand_2D_tangent_world,hand_2D_normal_world]))
			ICR_M3 = np.transpose(np.vstack([hand_2D_tangent_world_prev,hand_2D_normal_world_prev]))
			if np.abs(np.linalg.det(ICR_M1))>.0000001:
				ICR_B = np.linalg.solve(ICR_M1,hand_2D_midpoint_world_prev-hand_2D_midpoint_world)
				ICR_X = np.dot(ICR_M2,ICR_B)+hand_2D_midpoint_world

				print ([np.dot(ICR_M2,ICR_B)+hand_2D_midpoint_world,np.dot(ICR_M3,ICR_B)+hand_2D_midpoint_world_prev])
			else:
				ICR_X = None
		...
		if count%plot_time_ratio==0:
			...
			if ICR_X is not None:
				axs.scatter(ICR_X[0],ICR_X[1],color = 'red')
				# print(ICR_X)
			# for theta_index in range(len(theta_list)):
			# 	if measurement_count_list_plus[theta_index]>=measurement_threshold:
