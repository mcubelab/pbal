classdef PolygonMath
    ...
    methods(Static)
        ...
        %this function maps point pin on some solid body B
        %to their new locations
        %pout, after the solid body has translated 
        %given by the position of the origin of the rigid body
        %in the world frame 'position', and it's rotation 'theta'
        function pout=solid_body_position(position,theta,pin)
            s=size(pin);
        ...
        %given by the position of the origin of the rigid body
        %in the world frame 'position', and it's rotation 'theta'
        function vout=solid_body_velocity(velocity,theta,omega,pin)
            s=size(pin);
            Q=[0,-1;1,0];
            rot_matf=theta_to_rotmat(theta);
        ...
        %given by the position of the origin of the rigid body
        %in the world frame 'position', and it's rotation 'theta'
        function aout=solid_body_acceleration(acceleration,theta,omega,alpha,pin)
            s=size(pin);
            Q=[0,-1;1,0];
            rot_matf=theta_to_rotmat(theta);
...
%         %given by the position of the origin of the rigid body
%         %in the world frame 'position', and it's rotation 'theta'
%         function vout=solid_body_jacobian(velocity,theta,omega,pin)
%             s=size(pin);
%             rot_matf=theta_to_rotmat(theta);
% %             pout=rot_matf*(pin)+repmat(position,[1,s(2)]);
%         end
