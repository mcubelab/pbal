...
test_rigid_body.set_p_and_theta(-pout_temp,theta)


[x,y,Dx,Dy,Hx,Hy]=PolygonMath.rigid_body_position_derivatives(test_rigid_body.LocalPolygon.position,theta,pivot);
...
test_rigid_body.set_v_and_omega(dgeneralized(1:2),dgeneralized(3));

g=-100;
for count=1:10000

    [p,theta]=test_rigid_body.get_p_and_theta();
    [v,omega]=test_rigid_body.get_v_and_omega();
    
    generalized=[p;theta];
    dgeneralized=[v;omega];
    
    [x,y,Dx,Dy,Hx,Hy]=PolygonMath.rigid_body_position_derivatives(test_rigid_body.LocalPolygon.position,theta,pivot);
    
    [x_cm,y_cm,Dx_cm,Dy_cm,Hx_cm,Hy_cm]=PolygonMath.rigid_body_position_derivatives(generalized(1:2),generalized(3),r_cm);
...
%     Q2=-(Q2+Q2')*dgeneralized;
    ...
    [M,V] = test_rigid_body.LagrangeVM();
    
    BigMat=[[M,-Dx',-Dy'];[Dx,0,0];[Dy,0,0]];
    BigVec=[g*Dy_cm'+V;-dgeneralized'*Hx*dgeneralized;-dgeneralized'*Hy*dgeneralized];
    Accel=BigMat\BigVec;
   ...
   test_rigid_body.set_p_and_theta(generalized(1:2),generalized(3));
   test_rigid_body.set_v_and_omega(dgeneralized(1:2),dgeneralized(3));
   test_rigid_body.update_visualization();
   drawnow;
